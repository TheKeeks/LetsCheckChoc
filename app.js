...
class PhotoUploadManager {
  ...
  /** UUID v4 via Web Crypto API with getRandomValues fallback */
  _uuid() {
    if (typeof crypto !== 'undefined' && crypto.randomUUID) {
      return crypto.randomUUID();
    }
    // Fallback: use getRandomValues for cryptographic quality
  ...
  /** Find the hour index in cached forecast closest to now */
  _currentHourIdx() {
    const times = STATE._cachedMarine?.hourly?.time || [];
  ...
  /** Capture current conditions from existing global STATE */
  _captureConditions() {
    if (!STATE._cachedMarine || !STATE._cachedWind) return null;
    try {
      return buildForecastConditions(
        STATE._cachedMarine,
        STATE._cachedWind,
        STATE._cachedTideHiLo,
        this._currentHourIdx()
      );
    } catch (e) {
      return null;
    }
  ...
  /** Flatten a conditions object to a Firestore-safe plain structure */
  _conditionsToRecord(cond) {
    if (!cond) return null;
    return {
      swell_height:        cond.swell?.height ?? null,
      swell_period:        cond.swell?.period ?? null,
      swell_direction:     cond.swell?.direction ?? null,
      wind_speed:          cond.wind?.speed ?? null,
      wind_direction:      cond.wind?.direction ?? null,
      tide_height:         cond.tide?.height ?? null,
      tide_stage:          cond.tide?.stage ?? null,
      wind_offshore_score: cond.wind_offshore_score ?? null,
      blown_water_index:   cond.blown_water_index ?? null
    };
  }

  /**
   * Upload a photo to Cloud Storage and save metadata to Firestore.
   * @param {object} opts
   * @param {File}   opts.file
   * @param {string} opts.spotSlug
   * @param {string} opts.userNotes
   */
  async upload({ file, spotSlug, userNotes }) {
    if (this._uploading) throw new Error('An upload is already in progress.');
    const userId = window._fbUserId;
    if (!userId) throw new Error('Not authenticated — Firebase may still be initializing.');
    if (!file)   throw new Error('No photo file selected.');
    ...
    try {
      ...
      const storagePath = `user-uploads/${userId}/${spotSlug}/${ts}-${uuid}.${ext}`;
      ...
      // 2. Capture auto-conditions from current app state
      const autoConditions = this._captureConditions();

      // 3. Save metadata document to Firestore
      await fbFirestore.collection('photo_logs').add({
        userId,
        spot_slug:           spotSlug,
        photo_taken_at:      firebase.firestore.FieldValue.serverTimestamp(),
        storage_path:        storagePath,
        download_url:        downloadUrl,
        user_notes:          userNotes || '',
        reported_conditions: null,
        auto_conditions:     this._conditionsToRecord(autoConditions),
        status:              'pending',
...
const photoUploadManager = new PhotoUploadManager();

function initPhotoUploadSection() {
  const form = el('firebase-photo-form');
  if (!form) return;

  // Gracefully degrade if Firebase was not configured
  if (typeof firebase === 'undefined' || typeof fbStorage === 'undefined') {
    const sub = form.closest('.panel')?.querySelector('.fu-subtitle');
    if (sub) sub.textContent = 'Firebase not configured. See firebase-config.js for setup instructions.';
    const btn = el('fu-submit-btn');
    if (btn) btn.disabled = true;
    return;
  }

  const fileInput  = el('fu-photo-file');
  const previewEl  = el('fu-preview');
  const statusEl   = el('fu-status');
  const submitBtn  = el('fu-submit-btn');

  // Live preview when a file is chosen
  fileInput?.addEventListener('change', e => {
    const file = e.target.files[0];
    photoUploadManager._file = file || null;
  ...
  submitBtn?.addEventListener('click', async () => {
    if (!photoUploadManager._file) {
      if (statusEl) { statusEl.textContent = 'Please select a photo first.'; statusEl.className = 'fu-status fu-status-error'; }
      return;
    }
    if (!window._fbUserId) {
      if (statusEl) { statusEl.textContent = 'Firebase authentication is still initializing — please wait a moment and try again.'; statusEl.className = 'fu-status fu-status-error'; }
      return;
    }

    submitBtn.disabled = true;
    if (statusEl) { statusEl.textContent = 'Uploading…'; statusEl.className = 'fu-status fu-status-info'; }

    try {
      await photoUploadManager.upload({
        file:      photoUploadManager._file,
        spotSlug:  el('fu-spot')?.value || 'unknown',
        userNotes: el('fu-notes')?.value?.trim() || ''
      });
      if (statusEl) { statusEl.textContent = 'Photo uploaded! It will appear in the gallery after approval.'; statusEl.className = 'fu-status fu-status-success'; }
      // Reset form
      fileInput.value = '';
      photoUploadManager._file = null;
      if (el('fu-notes')) el('fu-notes').value = '';
      if (previewEl) previewEl.innerHTML = '';
    } catch (err) {
      console.error('Photo upload error:', err);
      if (statusEl) { statusEl.textContent = 'Upload failed: ' + err.message; statusEl.className = 'fu-status fu-status-error'; }
    } finally {
